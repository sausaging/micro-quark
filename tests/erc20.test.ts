...
import { QLMachine } from "../src/ql.ts";
import { reducers } from "../src/reducers.ts";
import { QL } from "../src/state.ts";
...
//   const charlieWallet = new Wallet(CHARLIE_ADDRESS);
  ...
  describe("Create and Mint", async () => {
    it("should create an account", async () => {
      ...
      const msgSender = ALICE_ADDRESS;
      const payload = {
        address: msgSender,
      };

      const signature = "hello world"
      ...
      const erc20Machine = mru.stateMachines.get<QLMachine>(
        STATE_MACHINES.QL
      );
